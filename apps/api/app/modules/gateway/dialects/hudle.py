"""Hudle — placeholder, awaiting their spec.

Hudle sells courts here, so they get a dialect. What they do not have yet is a
...
2. Handlers on `router`, at whatever paths they dictate. **Prefix every handler
   function with `hudle_`** (`hudle_availability`, not `availability`): operation ids
   are built from the tag and the function name, and every dialect shares the
   `gateway` tag, so a bare name silently collides with Playo's.
3. `HudleDriver`, implementing the six operations in `base.SandboxDriver` against
   `/gateway/…` — the advertised URL, never `/gateway/hudle/…`, so the sandbox
   exercises key-based routing the way Hudle will. All eight consistency scenarios
   then run against them without writing any.
4. Flip `is_ready` to True and set `driver=HudleDriver`.
...
DIALECT = Dialect(
    slug="hudle",
    label="Hudle",
    summary="Their own integration contract. Not built yet — we need Hudle's API "
