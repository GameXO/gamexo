...
/**
 * This app never sends `X-Tenant-ID`, and that is load-bearing.
 *
 * `VITE_TENANT_SLUG` bakes one fixed academy into the bundle. That was harmless
 * while every deployment served exactly one turf, and it is wrong now that turfs
 * sign themselves up: the header outranks everything else in tenant resolution
 * (see `app/tenancy/resolver.py::_plan_resolution`), so it would override the
 * academy a request actually belongs to. Both ways of getting here break:
 *
 *   * **Logging in.** Login carries no token, so the academy is found from the
 *     email in `account_directory`. A header naming the seeded demo tenant sends
 *     the lookup there instead, the user does not exist in it, and a correct
 *     password comes back "Incorrect email or password" — which is precisely how
 *     an emailed credential appears to be wrong when it is not.
 *   * **Every request after that.** The token's `tid` names the real academy; the
 *     header names the baked-in one; `auth/deps.py` sees the mismatch and 403s
 *     with "different academy".
 *
...
async function refreshOnce(): Promise<boolean> {
  ...
  refreshing ??= (async () => {
    try {
      // A platform token was minted by a different endpoint and carries a
      // different audience claim, so it has to be refreshed by its own — the
      // tenant refresh would reject it as issued for the wrong audience.
      const path = isPlatformSession()
...
async function request<T>(path: string, opts: RequestOptions = {}): Promise<T> {
  ...
  const send = async (): Promise<Response> => {
    ...
    // The one header that does name a tenant, and only a platform operator can
    // make it mean anything: the server honours it solely for a valid platform
...
export const api = {
   ...
   *  An email is still accepted, for accounts that predate usernames. */
   ...
   *  (see `models/user.py::PlatformAdmin`) and its token carries a different
   *  audience. `AuthProvider` falls back to this when the tenant login refuses. */
  platformLogin: (username: string, password: string) =>
    request<Ok<'/api/v1/platform/login', 'post'>>('/api/v1/platform/login', {
      method: 'POST',
      body: { username, password },
      anonymous: true,
    }),

  /** The operator themselves. Needs no tenant, which is the point — it is what
   *  can be called before an academy has been chosen to act inside. */
  platformMe: () =>
  ...
  /** Onboard an academy by hand — a venue that paid offline, or a demo.
   *
   *  Omitting the admin password makes the server generate one and return it, along
   *  with the counter login it creates alongside. Both are in the response once and
   *  nowhere else, so the caller must show them before navigating away. */
  createTenant: (body: {
    slug: string
    name: string
    admin: { email: string; full_name: string }
  }) =>
    request<Ok<'/api/v1/platform/tenants', 'post', 201>>('/api/v1/platform/tenants', {
      method: 'POST',
      body,
    }),

  /** Suspend, reactivate, or move an academy between plans.
   *
   *  Suspension bites immediately: its staff are refused at login and any live
   *  session stops renewing. An operator can still step inside with
   *  `X-Impersonate-Tenant`, which is what makes support on a suspended venue
   *  possible at all. */
  updateTenant: (
    tenantId: string,
    body: { status?: 'active' | 'trial' | 'suspended'; plan_tier?: 'starter' | 'growth' | 'pro' },
  ) =>
    request<Ok<'/api/v1/platform/tenants/{tenant_id}', 'patch'>>(
      `/api/v1/platform/tenants/${tenantId}`,
      { method: 'PATCH', body },
    ),

  /** Permanently delete an academy and everything it owns. **No undo.**
   *
   *  `confirm_name` must equal the academy's name exactly; the server checks it
   *  again rather than trusting the dialog, because a DELETE that needs only an id
   *  is one stray request away from an academy that no longer exists.
   *
   *  To stop an academy operating without destroying it, use `updateTenant` with
   *  `status: 'suspended'`. */
  deleteTenant: (tenantId: string, confirmName: string) =>
    request<Ok<'/api/v1/platform/tenants/{tenant_id}', 'delete'>>(
      `/api/v1/platform/tenants/${tenantId}`,
      { method: 'DELETE', body: { confirm_name: confirmName } },
    ),

  /** Reissue an academy account's password, returned once.
   *
   *  `username` defaults to that academy's `admin@{slug}`. This exists because there
   *  is no self-serve reset yet, so an owner whose welcome email never arrived has
   *  no other way back in. */
  resetTenantPassword: (tenantId: string, body: { username?: string } = {}) =>
    request<Ok<'/api/v1/platform/tenants/{tenant_id}/admin-password', 'post'>>(
